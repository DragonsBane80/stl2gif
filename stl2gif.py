...
def file_check(file):
    if not os.path.isfile(file):
        ...
        sys.exit(1)
    try:
        meshFile = mesh.Mesh.from_file(file)
        if meshFile.points.size == 0:
            print(f"Warning: STL file at {file} contains no mesh data.")
    except Exception as e:
        # Catches potential errors like file not found, permission errors,
        # or incorrect file format parsing issues.
        print(f"Failure: {file} is not a valid STL file. Error: {e}")
...
def make_rotating_gif(stl_path, duration_seconds=15, fps=20):
    ...
    renderer = pyrender.OffscreenRenderer(viewport_width=800, viewport_height=800, point_size=1.0)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-i', '--input', required=True, help='STL file to Read')

    args = parser.parse_args()
    stl = args.input
    file_check(stl)
    make_rotating_gif(stl)
